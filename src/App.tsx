...
export default function App() {
  ...
  const handlePDFExport = async () => {
    ...
    try {
      const scenarioName = selectedPresetIndex !== null ? PRESETS[selectedPresetIndex].name : "Custom Uploaded Room";
      const targetGoal = isCustomGoalActive ? customGoal : scenarioGoal;
      const imageUrl = getCurrentImageUrl();
      await exportReportToPDF(report, scenarioName, targetGoal, imageUrl, isCustomGoalActive);
    } catch (err: any) {
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {PRESETS.map((preset, idx) => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border text-xs transition-all flex items-start gap-3 ${selectedPresetIndex === idx
              ...
              <div className="relative">
                ...
                <label
                  ...
                  onDrop={handleDrop}
                  className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all block ${isDragOver
          ...
          <section className="lg:col-span-8 space-y-6">
            ...
            <div className="flex bg-slate-200/60 p-1.5 rounded-xl border border-slate-200/80 gap-1.5">
              <button
                ...
                className={`flex-1 py-3 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 cursor-pointer ${sessionMode === 'static'
              ...
              <button
                ...
                className={`flex-1 py-3 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 cursor-pointer ${sessionMode === 'live'
                  ...
                  <div className="border-b border-slate-200 px-5 py-3.5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200 self-start sm:self-auto">
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-all flex items-center gap-1.5 ${visionMode === 'normal'
                      ...
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-all flex items-center gap-1.5 ${visionMode === 'thermal'
                    ...
                    {getCurrentImageUrl() ? (
                      <div className="w-full h-full relative">
                        <img
                          ...
                          className={`w-full h-full object-cover transition-all duration-300 ${visionMode === 'thermal'
                        ...
                        {report && report.violations && report.violations.map((violation) => {
                          ...
                          return (
                            ...
                            >
                              <span className={`absolute inline-flex h-8 w-8 rounded-full ${isSelected ? 'scale-125 opacity-40 animate-pulse' : 'scale-100 opacity-20'} transition-all ${violation.severity === 'CRITICAL' ? 'bg-red-500' : 'bg-amber-500'
                                }`} />

                              <button
                                className={`w-6 h-6 rounded-full border shadow-lg font-bold text-[10px] flex items-center justify-center transition-transform ${severityColor} ${isSelected ? 'scale-110 ring-2 ring-white' : 'scale-100'
                                  }`}
                              >
                              ...
                              <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-slate-900 border ${violation.severity === 'CRITICAL' ? 'border-red-500' : 'border-amber-400'
                                } text-white p-3 rounded-lg shadow-xl text-xs w-64 transition-all duration-200 ${isSelected ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95 pointer-events-none'
                                }`}>
                                <div className="flex items-center justify-between gap-1 border-b border-slate-800 pb-1.5 mb-1.5">
                                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${violation.severity === 'CRITICAL' ? 'bg-red-950 text-red-400 border border-red-800' : 'bg-amber-950 text-amber-400 border border-amber-800'
                                    }`}>
                                    {violation.severity}
                                  </span>
                      ...
                      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                          <div className="space-y-0.5">
                            <span className="text-[10px] text-slate-500 font-semibold block uppercase tracking-wider">Enterprise Export</span>
                            <span className="text-[10px] text-slate-400 block leading-tight">Generate a formatted compliance report of this space.</span>
                          </div>
                          <button
                            onClick={handlePDFExport}
                            disabled={isGeneratingPDF}
                            className="flex items-center gap-1.5 py-1.5 px-3.5 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white rounded-lg transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                          >
                            {isGeneratingPDF ? (
                              <>
                        ...
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-100">
                          ...
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[8px] font-extrabold px-1.5 py-0.2 rounded ${report.structuralIntegrity.status === 'PASS' ? 'bg-emerald-100 text-emerald-800' :
                        ...
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 border border-slate-100">
                          ...
                          <div className="space-y-0.5">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className={`text-[8px] font-extrabold px-1.5 py-0.2 rounded ${report.environmentalCompliance.status === 'PASS' ? 'bg-emerald-100 text-emerald-800' :
                      ...
                      <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                        <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                          <button
                            ...
                            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all flex items-center gap-1.5 ${activeReportTab === 'violations'
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all flex items-center gap-1.5 ${activeReportTab === 'chat'
                                ? 'bg-white text-slate-900 shadow-sm font-bold'
                                : 'text-slate-500 hover:text-slate-800'
                              }`}
                          >
                        ...
                        {activeReportTab === 'violations' && (
                          <div className="flex items-center gap-2">
                            <button
                              onClick={handlePDFExport}
                              disabled={isGeneratingPDF}
                              className="flex items-center gap-1 py-1 px-2 text-xs font-semibold bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-md text-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            {report.violations.map((v) => {
                              ...
                              return (
                                <div
                                  ...
                                  onMouseLeave={() => setActiveMarkerId(null)}
                                  className={`p-4 border border-slate-200/80 rounded-xl transition-all duration-200 cursor-pointer ${severityClass} ${isHovered ? 'ring-1 ring-slate-400 shadow-md translate-x-1' : 'hover:bg-slate-50'
                                    }`}
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    ...
                                    <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded ${v.severity === 'CRITICAL' ? 'bg-red-100 text-red-800' :
